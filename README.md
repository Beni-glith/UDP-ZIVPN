...
Proyek ini menyediakan skrip otomatis untuk menghubungkan server ke panel ZIVPN menggunakan OpenVPN UDP, lengkap dengan service systemd agar tunnel otomatis berjalan ulang, menu terminal berwarna, HTTP API manager, serta bot Telegram untuk mengelola akun VPN langsung dari chat.
...
- **Install / Setup Awal**: Pasang dependensi, konfigurasi OVPN UDP, simpan config JSON, buat service tunnel.
...
- Setup bot dari menu: salin `telegram_bot.py` ke `/opt/zivpn-udp/` dan aktifkan service `zivpn-bot`.
- Instal dependensi bot:
```bash
pip install python-telegram-bot==20.7 requests
```
- Perintah yang didukung di Telegram:
  - `/start`, `/ping`, `/users`, `/add user pass days`, `/trial minutes`, `/delete user`, `/renew user days`, `/changepass user pass`, `/backup`, `/restore id`.
